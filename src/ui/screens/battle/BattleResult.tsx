...
const TINTS = ['linear-gradient(135deg,#3df0a0,#13c98a)', 'linear-gradient(135deg,#a98bff,#6a44e0)', 'linear-gradient(135deg,#4ea8ff,#6a5bff)', 'linear-gradient(135deg,#f5c542,#e8732c)', 'linear-gradient(135deg,#ff6e8a,#d23a5e)']
...
const CONF = ['#3df0a0', '#8b5cf6', '#f5c542', '#4ea8ff', '#ff6e8a']
  ...
  return (
    <div style={{ padding: '22px clamp(14px,2.4vw,28px) 32px', display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <section style={{
        ...
        background: iWon ? 'linear-gradient(180deg,rgba(47,226,138,.14),rgba(13,17,22,.55) 70%)' : 'linear-gradient(180deg,rgba(255,255,255,.04),rgba(13,17,22,.5))',
        border: `1px solid ${iWon ? 'rgba(47,226,138,.4)' : COLORS.border}`,
        ...
        <div style={{ position: 'absolute', top: '-46%', left: '50%', transform: 'translateX(-50%)', width: 420, height: 420, borderRadius: '50%', background: 'radial-gradient(circle,rgba(47,226,138,.18),transparent 65%)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginTop: 18, flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/play/arena')} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '13px 26px', borderRadius: 13, border: 0, cursor: 'pointer', fontFamily: FONTS.display, fontSize: 15, fontWeight: 700, color: '#06170f', background: GRADIENT, boxShadow: '0 12px 34px -10px rgba(47,226,138,.7)' }}>↻ Rematch</button>
          <button onClick={() => setVerifyOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '13px 22px', borderRadius: 13, border: '1px solid rgba(47,226,138,.32)', background: 'rgba(47,226,138,.08)', color: COLORS.green, cursor: 'pointer', fontFamily: FONTS.body, fontSize: 14, fontWeight: 600 }}>Verify · Provably Fair</button>
          ...
          {ranked.map((p, i) => {
            ...
            return (
              <div key={p.wallet} style={{
                ...
                background: isWinner ? 'linear-gradient(90deg,rgba(47,226,138,.10),rgba(255,255,255,.012))' : 'linear-gradient(180deg,rgba(255,255,255,.035),rgba(255,255,255,.012))',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 9 }}>
                    ...
                    {p.isMe && <span style={{ flex: 'none', padding: '2px 8px', borderRadius: 6, background: 'rgba(47,226,138,.14)', border: '1px solid rgba(47,226,138,.4)', fontSize: 9.5, fontWeight: 700, color: COLORS.green }}>YOU</span>}
                  </div>
      ...
      {vm.buybackTotal > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', padding: '16px 20px', borderRadius: 16, background: '#ffffff08', border: `1px solid ${COLORS.border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
            <span style={{ width: 36, height: 36, borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(47,226,138,.12)', border: '1px solid rgba(47,226,138,.32)', color: COLORS.green }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            ...
            <button onClick={() => navigate('/profile')} style={{ padding: '12px 22px', borderRadius: 12, border: 0, cursor: 'pointer', fontFamily: FONTS.display, fontSize: 14, fontWeight: 700, color: '#06170f', background: GRADIENT, boxShadow: '0 0 22px -6px rgba(47,226,138,.7)' }}>View inventory</button>
          </div>
        </div>
      )}
