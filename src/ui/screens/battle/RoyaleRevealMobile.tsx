      ...
      {players.map((p) => {
        ...
        return (
          <div key={p.wallet} {...(anchored ? { 'data-player-anchor': p.wallet } : {})} style={{
            ...
            {out ? (
              <span style={{
                fontFamily: FONTS.mono, fontSize: 9.5, fontWeight: 800, letterSpacing: '.06em', color: '#ff5e7a',
                padding: '2px 6px', borderRadius: 5, whiteSpace: 'nowrap',
                background: 'rgba(255,94,122,.12)', border: '1px solid rgba(255,94,122,.6)',
              }}>OUT R{p.eliminatedRound}</span>
            ) : (
