 ...
 *  `quietMark` hides the centred diamond + TCG wordmark: the stacked reveal puts three lines of
 *  text down the middle of the card and the mark collided with the one in the centre. */
export function CardBack({ width, height, accent, label, strong = false, quietMark = false }: { width: number; height: number; accent: string; label?: string; strong?: boolean; quietMark?: boolean }) {
  const glow = strong
