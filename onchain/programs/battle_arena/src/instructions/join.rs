...
pub fn handler(
    ...
    // Defensa en profundidad: aunque `initialize` ya fija `battle.oracle == TRUSTED_ORACLE`,
    // revalidamos aquí para que una batalla creada por una versión anterior del programa
    // (con un oráculo arbitrario almacenado) no pueda liquidarse con una atestación forjada.
